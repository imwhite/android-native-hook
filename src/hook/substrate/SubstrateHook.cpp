...
// matao add for repairing 'blx label'
static inline bool T$pcrel$bl_label(uint16_t *ic) {
    return (ic[0] & 0xf800) == 0xf000 && ((ic[1] & 0xd001) == 0xc000);
}
...
static size_t SubstrateHookFunctionThumb(void *symbol, void *replace, void **result) {
    if (symbol == NULL)
        return 0;
    ...
    if (
        ...
        SubstrateHookMemory code(arm + 1, sizeof(uint32_t) * 1);
    ...
    for (unsigned offset(0); offset != used / sizeof(uint16_t); ++offset) {
        if (T$pcrel$ldr(backup[offset])) {
            ...
            *--trailer = T$nop << 16 | T$bx(A$pc);
            ...
            // matao add for repairing 'blx label'
            uint32_t bl_target_val = (uint32_t)(area + offset) + 4 + jump;
            if (T$pcrel$bl_label(backup + offset)) {
                bl_target_val &= 0x0FFFFFFFC;
            }
            *--trailer = bl_target_val;
    ...
    {
        SubstrateHookMemory code(buffer, length);
    }
    ...
    *result = reinterpret_cast<uint8_t *>(buffer + pad) + 1;
    ...
    {
        SubstrateHookMemory code(area, used);
...
static size_t SubstrateHookFunctionARM(void *symbol, void *replace, void **result) {
    if (symbol == NULL)
        return 0;
    ...
    if (result != NULL) {
        ...
        if (backup[0] == A$ldr_rd_$rn_im$(A$pc, A$pc, 4 - 8)) {
            *result = (void* )backup[1];
            {
                SubstrateHookMemory code(symbol, used);
                tmp[0] = A$ldr_rd_$rn_im$(A$pc, A$pc, 4 - 8);
                tmp[1] = (uint32_t )replace;
                // utils::dx_hook_memcpy(arm, tmp, sizeof(tmp));
				*(uint64_t* )arm = *(uint64_t* )tmp;
            }
        ...
        // printf("\n[SubstrateHookFunctionARM] before mmap\n");
        uint32_t *buffer = (uint32_t* )utils::mmap(length);
        // printf("\n[SubstrateHookFunctionARM] after mmap, buffer = %p\n", buffer);
        if (buffer == NULL) {
            *result = NULL;
            return 0;
        }
        ...
        for (unsigned offset = 0; offset != used / sizeof(uint32_t); ++offset)
        {
            if (A$pcrel$r(backup[offset]))
            {
                ...
                *--trailer = (uint32_t )(area + offset) + 8;
                end -= 1;
            } else {
                buffer[start++] = backup[offset];
            }
        ...
        // printf("\n[SubstrateHookFunctionARM] 444\n");
        if (mprotect(buffer, length, PROT_READ | PROT_EXEC) == -1) {
            MSLog(MSLogLevelError, "MS:Error:mprotect():%d", errno);
            utils::munmap(buffer, length);
            *result = NULL;
            // printf("\n[SubstrateHookFunctionARM] return 0\n");
            return 0;
        }
        // printf("\n[SubstrateHookFunctionARM] 555\n");
    ...
    // printf("\n[SubstrateHookFunctionARM] 666\n");

    {
        SubstrateHookMemory code(symbol, used);
        // printf("\n[SubstrateHookFunctionARM] 666-1\n");
        // printf("\n[SubstrateHookFunctionARM] SubstrateHookMemory(%p, %d)\n", symbol, used);

        tmp[0] = A$ldr_rd_$rn_im$(A$pc, A$pc, 4 - 8);
        // printf("\n[SubstrateHookFunctionARM] 666-2\n");
        tmp[1] = (uint32_t )replace;
        // printf("\n[SubstrateHookFunctionARM] 666-3, arm: 0x%.8X, 0x%.8X\n", arm[0], arm[1]);
        // printf("\n[SubstrateHookFunctionARM] 666-3, arm: 0x%.8X, 0x%.8X\n", tmp[0], tmp[1]);
        *(uint64_t* )arm = *(uint64_t* )tmp;
//        printf("\n[SubstrateHookFunctionARM] 666-3-2\n");
//        printf("\n[SubstrateHookFunctionARM] 666-3-3\n");


//        utils::dx_hook_memcpy(arm, tmp, sizeof(tmp));


        // printf("\n[SubstrateHookFunctionARM] 666-4\n");
    }

    // printf("\n[SubstrateHookFunctionARM] 777\n");
    if (MSDebug) {
        char name[16];
        sprintf(name, "%p", area);
        MSLogHexEx(area, used + sizeof(uint32_t), 4, name);
    ...
    // printf("\n[SubstrateHookFunctionARM] return %d\n", used);
	return used;
}

static size_t SubstrateHookFunction(void *symbol, void *replace, void **result) {
    if (MSDebug)
        MSLog(MSLogLevelNotice, "SubstrateHookFunction(%p, %p, %p)\n", symbol, replace, result);
    if ((reinterpret_cast<uintptr_t>(symbol) & 0x1) == 0)
        return SubstrateHookFunctionARM(symbol, replace, result);
    else
        return SubstrateHookFunctionThumb(reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(symbol) & ~0x1), replace, result);
}
#endif
...
static size_t MSGetInstructionWidthIntel(void *start) {
    hde64s decode;
    return hde64_disasm((const void* )start, &decode);
}
    ...
    for (size_t offset(0), width; offset != used; offset += width) {
        ...
        if (backup[offset] == 0xe8) {
            ...
            if (relative == 0) {
                ...
                int regIndex = GetPcTrunkRegIndex((uint8_t* )destiny);
                // printf("regIndex: %d\n", regIndex);
                if (regIndex >= 0) {
                    length += 5;
                } else {
        ...
        for (size_t offset(0), width; offset != used; offset += width) {
            ...
            if (backup[offset] == 0xe8) {
                ...
                if (relative == 0)
                    ...
                    int regIndex = GetPcTrunkRegIndex((uint8_t* )destiny);
                    if (regIndex >= 0) {
                        MSWrite<uint8_t>(current, 0xB8 + regIndex);
                        MSWrite<int32_t>(current, (int32_t )(intptr_t )(area + offset + decode.len));
                    } else {
...
void MSHookFunction(void *symbol, void *replace, void **result) {
     SubstrateHookFunction(symbol, replace, result);
}

static bool l_call_from_apk = true;

void dx_hook_make_as_system_caller(void) {
    l_call_from_apk = false;
}

DXRetStatus dx_hook_hookfun(void *symbol, void *replace, void **origin) {
    int curent_abi = utils::get_curenv_abi();

    if (HKABI_I386 == utils::get_loaded_lib_abi("/linker")) {
        if (l_call_from_apk) {
            if (curent_abi == HKABI_ARM
                && utils::is_addr_executable((uintptr_t )symbol)) {
                return DX_ABI_NOT_MATCH;
            }
        } else {
            if (curent_abi == HKABI_I386
                && !utils::is_addr_executable((uintptr_t )symbol)) {
                return DX_ABI_NOT_MATCH;
            }
        }
